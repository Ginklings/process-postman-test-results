...
async function lookForExistingComment(octokit) {
  ...
  while (hasMoreComments) {
    ...
    if (commentsResponse.status == 200) {
      if (commentsResponse.data) {
        ...
        if (existingComment) {
          core.info(`An existing postman test results comment (${existingComment.id}) was found and will be udpated.`);
  ...
  core.info('An existing postman test results comment was not found, will create a new one instead.');
